    ...
    static void apply(MetricDeltaHistory history,
        ...
        for (MetricDeltaHistory.Interval interval : history.getIntervals()) {
            ...
            for (int metric = 0; metric < AeeemMetricAccess.FEATURE_COUNT; metric++) {
                ...
                // Per D'Ambros et al. 2012 (Section 3.6, Eq. 17 & 21), each class that
                // changes between two versions (delta > 0) gets the system entropy:
                // LDHH(i, j) = H^0(j) / (phi_2 * (C + 1 - j)) when delta > 0, and 0 otherwise.
