...
/**
 * Presenter: renders a parsed {@link DatasetTable} back out as CSV or ARFF
 * text so a dataset stored in one format can still be downloaded in the
 * other. Pure in-memory formatting with no I/O of its own, so it lives next
 * to the controller it serves rather than in infrastructure.
 */
