...
public final class GitNumstatParser {
    ...
    public static List<FileChange> parseNameStatus(String output, long defaultChangedLines) {
        if (output == null || output.trim().isEmpty()) {
            return Collections.emptyList();
        }
        List<FileChange> changes = new ArrayList<>();
        ...
        for (String line : output.split("\\R")) {
            ...
            if (status.startsWith("R") || status.startsWith("C")) {
                if (fields.length >= 3) {
                    String oldPath = normalize(unquote(fields[1].trim()));
                    String newPath = normalize(unquote(fields[2].trim()));
                    if (isJava(oldPath) || isJava(newPath)) {
                        changes.add(new FileChange(commit, oldPath, newPath, defaultChangedLines));
                    }
                }
            } else if (status.startsWith("M") || status.startsWith("A") || status.startsWith("D")) {
                String path = normalize(unquote(fields[1].trim()));
                if (isJava(path)) {
                    changes.add(new FileChange(commit, path, path, defaultChangedLines));
                }
            }
        }
        return changes;
    ...
    public static final class FileChange {
        ...
        public FileChange(String commit, String oldPath, String newPath, long changedLines) {
            this.commit = commit;
