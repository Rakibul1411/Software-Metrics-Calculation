...
class AeeemHistoryCalculatorsTest {
    ...
    @Test
    void wchuWeightsPositiveChangesWithAlphaAndUnchangedMetricsAreZero() {
        ...
        // demo.Missing/demo.New never exist at both endpoints of any single
        // interval (D(i,j) = -1 throughout), so they earn no credit at all.
        assertEquals(0d, release.get("demo.Missing").getWchuWmc(), 1.0e-12);
        assertEquals(0d, release.get("demo.New").getWchuWmc(), 1.0e-12);
        // demo.A survives both intervals, but dit never moved (delta = 0 in both),
        // so its WCHU is 0.0 per Eq. 8-10 of D'Ambros et al. 2012.
        assertEquals(0.0d, release.get("demo.A").getWchuDit(), 1.0e-12);
    }
